    ...
    <main class="flex-grow-1 d-flex flex-column" style="height: 100vh; overflow: auto;">
        ...
        <div class="flex-grow-1">
            <div class="container-fluid p-3">
                ...
                <p class="mb-3">Daftar tindakan.</p>
            </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped mb-0">
                            ...
                            <tbody>
                                @forelse(($tindakans ?? collect()) as $i => $t)
                                    <tr>
                                        <td>{{ $i + 1 }}</td>
                                        <td>{{ $t->idtindakan ?? ($t->id ?? '-') }}</td>
                                        <td>{{ $t->nama_tindakan ?? ($t->name ?? '-') }}</td>
                                        <td>{{ $t->harga ?? '-' }}</td>
                                        <td>
                        ...
                        <table class="table table-striped-hover mb-0">
                            ...
                            <tbody>
                                @forelse(($tindakans ?? collect()) as $i => $item)
                                    <tr>
                                        <td>{{ $i + 1 }}</td>
                                        <td>{{ $item->id ?? ($item->idtindakan ?? '-') }}</td>
                                        <td>{{ $item->nama ?? ($item->name ?? '-') }}</td>
                                        <td>{{ $item->deskripsi ?? ($item->description ?? '-') }}</td>
                                        <td>
                                            <div class="d-grid gap-2 d-md-block">
                                                <a href="#"
                                                    class="btn btn-sm btn-info p-1 px-2"
